        ...
        for statement in &mut block.0 {
            ...
            match statement {
                ...
                Statement::GenericFor(generic_for) => {
                    let local_count = generic_for.res_locals.len();
                    for (i, res_local) in generic_for.res_locals.iter().enumerate() {
                        if local_count == 2 && i == 0 {
                            self.name_local("key", res_local);
                        } else if local_count == 2 && i == 1 {
                            self.name_local("val", res_local);
                        } else {
